...
// Note: InertiaPlugin requires GSAP Club membership. 
// We load it from CDN as fallback if not available via npm.
// For now, we'll use a simplified elastic return instead.
...
export default function DotGrid({
  ...
  useEffect(() => {
    ...
    // Pause when dot-grid scrolls off-screen
    const container = document.getElementById('dot-grid-bg')
  ...
  // Build grid — deferred to avoid blocking first paint
  useEffect(() => {
    const scheduleGrid = () => {
      if (typeof requestIdleCallback !== 'undefined') {
        requestIdleCallback(() => buildGrid())
      } else {
  ...
  return (
    <div id="dot-grid-bg">
      <section className="dot-grid" style={{ width: '100%', height: '100%', position: 'absolute', top: 0, left: 0 }}>
